...
*"The mean is \_\_\_"*. (Where the blank is the mean.)

```{r}

find_mean2 <- function(mynumbers) {
  my_mean <- mean(mynumbers)
  return(paste0("The mean is ", my_mean))
}


find_mean2(c(4,5,6,7,8))
...
airline_count_by_airport <- function(airportcode) {
  results <- flights %>% 
    filter(origin == airportcode) %>% 
...
allstates_wide <- allstates_wide %>% 
  select(-education_totalM,
         -education_bachelorsM,
         -education_gradprofessM)
...
allstates_wide <- allstates_wide %>%
  mutate(
    pct_college_grads = round_half_up((education_bachelorsE + education_gradprofessE) / education_totalE * 100, 2)
  )
...
map_collegegradpcts <- tm_shape(allstates_wide) +
  tm_polygons("pct_college_grads") +
  tm_layout(main.title = "Percent of State Populations with a Bachelor's or Higher",
